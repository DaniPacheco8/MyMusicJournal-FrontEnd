...
import { isValidNotes, isValidRating } from '../../utils/validators';
...
export const ConcertFormModal = ({ isOpen, entry, onClose, onSuccess }) => {
  ...
  useEffect(() => {
    const fetchConcerts = async () => {
      try {
        ...
        // Error fetching concerts - will be handled by validation
      }
    };
  ...
  useEffect(() => {
    ...
    if (filterYear) {
      filtered = filtered.filter(
        (concert) =>
          new Date(concert.date).getFullYear().toString() === filterYear
      );
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const errorMessage =
        error.message || 'Failed to save concert. Please try again.';
